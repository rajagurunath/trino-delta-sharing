        ...
        implements ParquetDataSource
{
    ...
    public FileParquetDataSource(
            File path,
            ParquetReaderOptions options)
            throws IOException
    {
        this.id = new ParquetDataSourceId(path.toString());
        this.input = new RandomAccessFile(path, "r");
        this.options = requireNonNull(options, "options is null");
        this.estimatedSize = input.length();
    }
    ...
    public Slice readTail(int length)
    {
        ...
        Slice tailSlice = readFully(estimatedSize - length, length);
